...
func main() {
	...
	// Slices support a “slice” operator with the syntax slice[low:high], includes the "low" but not inlcuding the "high".
	l := s[2:5]
	fmt.Println("sl1:", l)
	...
	// Can use append instead of the two makes
	twoDD := make([][]int, 3)
	for ii := 0; ii < 3; ii++ {
		for jj := 0; jj < ii+1; jj++ {
			twoDD[ii] = append(twoDD[ii], ii+jj)
		}
	}
	fmt.Println("2dd:", twoDD)

	// The below will produce an error because the inner slice has no length defined and when trying to give value to an index that doesn't exist it produces an error:
	// twoDD := make([][]int, 3)
	// for ii := 0; ii < 3; ii++ {
	// 	for jj := 0; jj < ii+1; jj++ {
	// 		twoDD[ii][[jj]] = ii + jj
